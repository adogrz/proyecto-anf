...
{
    // Normalizamos el nombre para evitar fallos por mayúsculas o tildes
    $buscar = $this->normalizarTexto($nombreCuenta);

    $detalle = $estado->detalles->first(function ($d) use ($buscar) {
        if (!$d->catalogoCuenta) return false;

        $nombreCatalogo = $this->normalizarTexto($d->catalogoCuenta->nombre_cuenta);

        // Coincidencia flexible: si contiene parte del nombre buscado
        return str_contains($nombreCatalogo, $buscar);
    });

    return $detalle?->valor ?? 0.0;
}
